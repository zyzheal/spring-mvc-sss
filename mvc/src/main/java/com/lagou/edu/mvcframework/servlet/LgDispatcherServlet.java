...
public class LgDispatcherServlet extends HttpServlet {
    ...
    //存储方法上权限的值，key：访问路径 value 权限上的values
    private Map<String, List> setSuthority = new HashMap<String, List>();
    ...
    private void initHandlerMapping() {
        ...
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                LagouSecurity annotation1 = method.getAnnotation(LagouSecurity.class);
                String[] value = annotation1.value();
                //添加到map
                setSuthority.put(url,Arrays.asList(value));
            }
...
//        System.out.println("handler = " + handler );
        ...
        for (Map.Entry<String, List> entry : setSuthority.entrySet()){
            String key = entry.getKey();
            if(pathInfo.equalsIgnoreCase(key)) {
                List values = entry.getValue();
                if (values.contains(username)) {
                    resp.getWriter().write("hello " + username);
                    System.out.println("url = " + key + ", pattern = " + handler.getPattern().toString());
                    continue;
                } else {
                    System.out.println("url = " + key + ", username = " + username);
                    resp.getWriter().write("Sorry, u have  no access");
                    return;
                }
            }else{
                continue;
